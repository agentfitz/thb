...
<div id="main_content">
	...
	<div id="home_primary">	
		<?php 							
			$list_cat_ids = array(5,3,4);
			$list_cat_labels = array('mind','recipes','beauty');	
			$list_cat_names = array('Mind &amp; Body','Recipes','Natural Beauty');							
		...
		<div id="recent-posts" class="home-recent clearfix <?php if($i == count($list_cat_ids) - 1) echo 'posts-wrap-last' ?>">			
			...
			<h1><a href="/category/<? echo $list_cat_labels[$i] ?>"><? echo $list_cat_names[$i] ?></a></h1>
	
			<?php
				$start = 2; //Set the starter count		
				...
				<?php if(is_multiple($start, 2)) : /* if the start count is a multiple of 3 */ ?>
					<!--BEGIN .hentry-wrap -->
					...
					<?php endif; ?>
						...
						<div class="post-thumb">
							<a title="<?php printf(__('Permanent Link to %s', 'framework'), get_the_title()); ?>" href="<?php the_permalink(); ?>"><img src="/wp-content/uploads/img/<?php echo the_ID() . '-thumb.jpg'; ?>" width="325" height="211" /></a>
						</div>
						...
						<div class="entry-meta entry-header">
								<?php /* <span class="published"><?php the_time( get_option('date_format') ); ?></span>
					...
					<?php if(is_multiple($finish, 2) || $post_count == $finish) : /* if the finish count is a multiple of 3 or equals the total posts */  ?>
						</div><!--END .hentry-wrap-->  
					<?php endif; ?>
